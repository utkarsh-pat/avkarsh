...
import {
  CalendarDays, CheckCircle2, CircleAlert, Clock3, DoorOpen, IndianRupee,
  ListFilter, MessageSquareText, Search, Sparkles, UsersRound, Wrench, X,
} from "lucide-react";
...
export function SuperViewBoard({ propertyId, inventoryMode, currencyCode, units, allocations, tasks, todayIso, requested }: Props) {
  ...
  return <section className="super-view" aria-labelledby="super-view-title">
    <header className="super-view-heading"><div><p className="eyebrow">LIVE OPERATIONS</p><h2 id="super-view-title">Super View</h2><p>Occupancy and physical readiness stay separate, so the front desk always knows what is sellable.</p></div><nav aria-label="Super View mode">{[["cards", "Live cards"], ["timeline", "Timeline"], ["calendar", "Calendar"]].map(([value, label]) => <Link key={value} href={href({ ...requested, view: value })} aria-current={view === value ? "page" : undefined}>{label}</Link>)}</nav></header>
    ...
    {!visible.length ? <div className="super-view-empty"><Search /><h3>No units match these filters</h3><p>Clear the filters to return to the live property board.</p><Link className="button primary" href="?view=cards&filter=all">Clear filters</Link></div> : view === "cards" ? <div className="super-view-groups">{groups.map((group) => <section key={group.floor}><header><div><strong>{group.floor}</strong><span>{group.items.length} {inventoryMode}</span></div><span>{group.items.filter((item) => item.occupancyState === "occupied").length} occupied</span></header><div className="super-view-card-grid">{group.items.map(({ unit, current, next, openTasks, occupancyState }) => <article className="super-view-card" key={unit.id} data-occupancy={occupancyState} data-operation={unit.operational_state}>
      <header><div><span><DoorOpen /></span><div><small>{unit.category || pretty(unit.unit_kind)}</small><h3>{unit.unit_code}</h3><p>{unit.display_name}</p></div></div><div className="unit-state-stack"><b>{pretty(occupancyState)}</b><span>{unit.operational_state === "ready" ? <Sparkles /> : unit.operational_state === "maintenance" ? <Wrench /> : <Clock3 />}{pretty(unit.operational_state)}</span></div></header>
